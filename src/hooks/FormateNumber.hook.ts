import { FormateNumber, NumberEdit } from '../interface/FormateNumber.interface'
import { global } from '../config'

export const useFormateNumber = (num: number, currency: string): string | number => {
...
export const useFormateNumberSapn = (num: number, currency: string): FormateNumber => {
  const simbol = currency || global.currencyDefault
...
const numberFormat = (edit: NumberEdit): string => {
  ...
  const prec: number = (typeof number !== 'number')
    ? number % 1 === 0
      ? 0
      : 2
  ...
  const toFixedFix = (n:number, prec:number): string | number => {
    ...
    return (+`${Math.round(parseFloat(`${+arr[0]}e${sig}${+arr[1] + prec}`))}e-${prec}`).toFixed(prec)
  }
